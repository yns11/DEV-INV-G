...
This service exists to assemble one thing — the digest the model is allowed to
see. That assembly *is* the safety property. The model has no database handle,
no tools and no browsing; it can only be right or wrong about what this file
chose to put in front of it.

The digest is a summary, never the raw campaign. Not to save money, but because
a campaign is hundreds of thousands of rows and no answer improves by drowning
the twenty numbers that matter in them. What goes in is what a human would
gather before answering the same question: where the campaign is, what has been
counted, where the money is, and what is blocking.
"""
...
from ..ai.assistant import Attachment, CampaignAssistant
...
class AssistantService:
    ...
    def ask(
        ...
        *,
        ...
        """Answer *question* about *campaign*.

        Read-only by construction: nothing here writes a quantity, a cause or a
        status. The only trace left is the audit line saying a question was
        asked — which is worth keeping, because an answer that turns out to be
        wrong needs to be findable.
        """
        ...
        self.ctx.record(
            ...
            summary=f"Question à l'assistant : {question[:120]}",
        )
        return answer.as_dict()

    # ------------------------------------------------------------- the digest

    def context(self, campaign: Campaign) -> dict[str, Any]:
        """The campaign digest handed to the model.
        ...
        digest["plusGrosÉcarts"] = [
            ...
            for v in analysis.top_variances(campaign, limit=_TOP_VARIANCES)
        ]
    ...
    def _zones(self, campaign: Campaign) -> list[dict[str, Any]]:
